...
from src.interactive_automation_mcp.security import SecurityManager, RateLimitData
...
class TestRateLimitData:
    ...
    def test_clean_old_calls(self):
        ...
        assert data.get_recent_call_count() == 2
...
class TestSecurityManagerInit:
    ...
    def test_security_manager_initialization(self, security_manager):
        ...
        assert security_manager.max_calls_per_minute == 60
        assert security_manager.max_sessions == 50
        assert len(security_manager.blocked_command_patterns) > 0
        assert len(security_manager.allowed_base_paths) >= 4
...
class TestInputValidation:
    ...
    def test_block_dangerous_interactive_input(self, security_manager, malicious_inputs):
...
class TestEnvironmentValidation:
    ...
    def test_block_protected_environment_vars(self, security_manager, sample_environment_vars):
    ...
    def test_validate_environment_with_injection(self, security_manager, malicious_inputs):
...
class TestToolCallValidation:
    ...
    def test_validate_safe_execute_command(self, security_manager):
        ...
        assert security_manager.validate_tool_call("execute_command", arguments) is True
    ...
    def test_block_dangerous_execute_command(self, security_manager):
        ...
        for args in dangerous_args:
            assert security_manager.validate_tool_call("execute_command", args) is False
    ...
    def test_validate_other_tool_calls(self, security_manager):
        ...
        assert security_manager.validate_tool_call("list_sessions", arguments) is True
        assert security_manager.validate_tool_call("destroy_session", arguments) is True
        assert security_manager.validate_tool_call("get_screen_content", arguments) is True
    ...
    def test_rate_limiting_in_tool_validation(self, security_manager):
        ...
        # Max out rate limit
        for _ in range(60):
            security_manager.validate_tool_call("execute_command", arguments, "test_client")
        
        # Next call should be blocked due to rate limit
        assert security_manager.validate_tool_call("execute_command", arguments, "test_client") is False
...
class TestAuditLogging:
    ...
    def test_write_audit_log(self, security_manager, mock_audit_log_path):
        ...
        with open(mock_audit_log_path, 'r') as f:
...
class TestSecurityIntegration:
    ...
    def test_complete_security_workflow(self, security_manager):
        ...
        result = security_manager.validate_tool_call(
            "execute_command", safe_arguments, "integration_test_client"
        )
    ...
    def test_multi_layer_security_blocking(self, security_manager):
        ...
        result = security_manager.validate_tool_call(
            "execute_command", malicious_arguments, "malicious_client"
        )
    ...
    def test_security_across_different_clients(self, security_manager):
        ...
        assert security_manager.validate_tool_call("execute_command", safe_args, "client1") is True
        assert security_manager.validate_tool_call("execute_command", safe_args, "client2") is True
        assert security_manager.validate_tool_call("execute_command", safe_args, "client3") is True
