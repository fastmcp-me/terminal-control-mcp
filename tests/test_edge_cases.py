...
from src.interactive_automation_mcp.main import execute_command, send_input, get_screen_content, destroy_session
from src.interactive_automation_mcp.models import ExecuteCommandRequest, SendInputRequest
...
class TestRateLimitingEdgeCases:
    ...
    def test_concurrent_rate_limiting(self, security_manager):
        ...
        assert successes <= 60  # Max allowed per minute
...
class TestEnvironmentEdgeCases:
    ...
    def test_environment_case_sensitivity(self, security_manager):
        ...
        mixed_case_env = {
            "path": "/malicious",     # Lowercase
            "Path": "/malicious",     # Title case  
            "HOME": "/tmp/fake",      # This should be blocked
            "home": "/tmp/fake2",     # This might not be
        }
...
class TestAsyncEdgeCases:
    ...
    async def test_rapid_session_creation_destruction(self, mock_context):
        ...
        try:
            ...
            for session_id in session_ids:
                destroy_request = {"session_id": session_id}
    ...
    async def test_timeout_edge_cases(self, mock_context):
        ...
        if result.success:
            # Clean up
            destroy_request = {"session_id": result.session_id}
            await destroy_session(destroy_request, mock_context)
    ...
    async def test_concurrent_operations_same_session(self, mock_context):
        ...
        try:
            ...
            # Get screen content
            screen_request = {"session_id": session_id}
            tasks.append(get_screen_content(screen_request, mock_context))
            
            # Send input
            input_request = {"session_id": session_id, "input_text": "test"}
