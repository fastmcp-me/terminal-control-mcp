...
async def main():
    ...
    try:
        ...
        if not start_result.success or not start_result.session_id:
            print(f"✗ {test_name} failed: Could not start session. Error: {start_result.error}")
            return
        ...
        if not interaction_result.get("success"):
            print(f"✗ {test_name} failed: Interaction failed. Error: {interaction_result.get('error')}")
